...
class TestIrrigation:
    ...
    def test_get_last_irrigation(self, irrigation_service):
        irrigation_service.get_last_irrigation().assert_ok_or_skip_404("No irrigation data in prod for configured farm")

    @pytest.mark.flaky
    @allure.story("Get Program Schemes")
    def test_get_program_schemes(self, irrigation_service):
        irrigation_service.get_program_schemes().assert_ok_or_skip_404("No program schemes in prod for configured farm")

    @pytest.mark.flaky
    @allure.story("Get Recipes")
    def test_get_recipes(self, irrigation_service):
        irrigation_service.get_recipes().assert_ok_or_skip_404("No recipes in prod for configured farm")
